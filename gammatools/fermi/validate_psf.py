...
os.environ['CUSTOM_IRF_NAMES']='P7SOURCE_V6,P7SOURCE_V6MC,P7SOURCE_V9,P7CLEAN_V6,P7CLEAN_V6MC,P7ULTRACLEAN_V6,P7ULTRACLEAN_V6MC,P6_v11_diff,P7SOURCE_V6MCPSFC,P7CLEAN_V6MCPSFC,P7ULTRACLEAN_V6MCPSFC'
...
class PSFValidate(object):
    ...
    def __init__(self,opts):
        ...
        if opts.output_dir is None: self.output_dir = os.getcwd()
        else: self.output_dir = opts.output_dir
    ...
    def build_models(self):
        ...
        for imodel, ml in enumerate(self.models):
            ...
            for icth in range(len(cth_range)):
                ...
                m = psf_model.PSFModelLT(self.opts.ltfile,irfm,
                                         nbin=300,
...
#    def fill_hist(self,data,theta_edges,egy_range,cth_range,
#                  phase=None):
#        h = Histogram(theta_edges)
#
#        mask = self.get_mask(data,egy_range,cth_range,phase)
#        
#        h.fill(data['dtheta'][mask]*180./np.pi)
#        return h
...
#            ax.set_yscale('log')
    ...
    #    ax.set_title(title)
        ...
        hsignal.errorbar(True,True,ax=axes[0],fmt='o',label='signal')
        hbkg.errorbar(True,True,ax=axes[0],fmt='o',label='bkg')

        for i, h in enumerate(hmodel):
            h.errorbar(False,False,ax=axes[0],fmt='-',
                       label=self.model_labels[i],
                       color=self.irf_colors[i],
                       linewidth=2)

            hresid = copy.deepcopy(hsignal)
            hresid -= h
            hresid = hresid.divide(h)

            hresid.errorbar(ax=axes[1],
                            label=self.model_labels[i],
                            color=self.irf_colors[i],
                            linewidth=2)
...
#                                 subplot_kw={'xscale' : 'sqrt', 'exp' : 2.0})
        ...
        hsignal.errorbar(ax=axes[0],color='b')
        hbkg.errorbar(False,False,ax=axes[0],fmt='--',label='bkg',
                      color='k')
